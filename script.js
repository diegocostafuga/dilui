...
import {
    PRODUTOS_PRECADASTRADOS,
    UNIDADES,
    PROPORCOES_SUGERIDAS,
    INCOMPATIBILIDADES,
    calcular,
    converterQuantidade,
    obterNomeProduto,
...
// Renderiza o expansível "Por que estamos avisando?" com a lista de fontes
// técnicas daquele par. Usa <details>/<summary> nativo — sem JS extra.
// Chamada de dentro do template de cada item do banner.
function _renderFontesDetails(fontes) {
    if (!fontes || fontes.length === 0) return '';

    const itens = fontes.map(f => {
        const tipoLabel = _LABEL_TIPO_FONTE[f.tipo] || f.tipo;
        const conteudo = f.url
            ? `<a href="${f.url}" target="_blank" rel="noopener noreferrer">${f.referencia}</a>`
            : f.referencia;
        return `<li class="ib-fonte">
            <span class="ib-fonte-tipo">${tipoLabel}</span>
            <span class="ib-fonte-ref">${conteudo}</span>
        </li>`;
    }).join('');

    return `
        <details class="ib-fontes-details">
            <summary class="ib-fontes-summary">Por que estamos avisando?</summary>
            <ul class="ib-fontes-list">${itens}</ul>
        </details>
    `;
}
...
function renderizarBannerIncompatibilidade(container, avisos, opts = {}) {
    ...
    const disclaimerHtml = incluirDisclaimer ? `
        <p class="ib-disclaimer">
            Compilação técnica de fontes públicas — não substitui a leitura da embalagem nem orientação profissional. Pendente de revisão por especialista antes da v1 pública.
        </p>
    ` : '';
